...
def pullTexts(text):
    ...
    #get raw text and language information from parameters
    raw = text

    #convert texts to textBlobs
    obj = TextBlob(raw)

    #detect the language so user does not have to input
    ...
    line = [text,lang,len1,pol,subj,counts[0],counts[1],counts[2],tPol,tSubj,tCounts[0],tCounts[1],tCounts[2]]
...
def writeData(line1,line2):
    ...
    with open('userinterface/markov/enfrdata.csv','a') as fd:
        writer = csv.writer(fd)
        writer.writerow(lineCombined)

def analyze(texts):
    line1 = pullTexts(texts[0])
    line2 = pullTexts(texts[1])

    return [line1[3],line1[4],line1[8],line1[9],line2[3],line2[4]]
